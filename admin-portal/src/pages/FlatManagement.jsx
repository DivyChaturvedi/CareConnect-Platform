...
import { getFlats, createFlat } from "../services/flatService";
import { getBlocks } from "../services/blockService";

/* ------------------------------------------------------------------ */
/* Design tokens — shared language with Dashboard / Login / Blocks     */
/* ------------------------------------------------------------------ */
const COLORS = {
    bg: "#0a0e1a",
...
function FlatManagement() {
    ...
    useEffect(() => {
        Promise.all([fetchBlocks(), fetchFlats()]).finally(() => setLoading(false));
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({ block: "", flat_number: "", floor: "" });
            await fetchFlats();
        } finally {
            setSubmitting(false);
        }
    ...
    const blockName = (id) => blocks.find((b) => String(b.id) === String(id))?.name;
        ...
        <div style={styles.page}>
            ...
            <div style={styles.container}>
                ...
                <div className="cc-fade-in" style={{ ...styles.header, animationDelay: "0ms" }}>
                    ...
                    <div>
                        <h1 className="cc-heading">Flat Management</h1>
                        <p style={styles.subText}>Register units within your blocks</p>
                    </div>
                </div>
                ...
                <TiltCard max={3} className="cc-fade-in cc-card" style={{ animationDelay: "80ms" }}>
                    ...
                    <h3 style={styles.cardTitle}>New Flat</h3>
                    ...
                    <form onSubmit={handleSubmit} style={{ position: "relative", zIndex: 1 }}>
                        ...
                        <div style={styles.formRow}>
                            <div style={{ flex: 1 }}>
                                <label className="cc-label">Flat number</label>
                                <input
                                    type="text"
                                    className="cc-input"
                                    placeholder="e.g. 302"
                            ...
                            <div style={{ flex: 1 }}>
                                <label className="cc-label">Floor</label>
                                <input
                                    type="number"
                                    className="cc-input"
                                    placeholder="e.g. 3"
                ...
                <div className="cc-fade-in" style={{ animationDelay: "160ms" }}>
                    <h3 style={{ ...styles.cardTitle, margin: "8px 4px 14px" }}>
                        Existing Flats {!loading && <span className="cc-count-badge">{flats.length}</span>}
                    </h3>
                        ...
                        <div style={styles.grid}>
                            {flats.map((flat, i) => (
                                <TiltCard
                                    key={flat.id}
                                    max={12}
                                    className="cc-card cc-flat-card"
                                    style={{ animationDelay: `${200 + i * 50}ms` }}
                                >
                                    <div className="cc-card-glow" />
                                    <div className="cc-flat-key">🔑</div>
                                    <div style={{ position: "relative", zIndex: 1 }}>
                                        <h3 className="cc-flat-number">{flat.flat_number}</h3>
                                        <p className="cc-flat-meta">
                                            {flat.floor !== undefined && flat.floor !== null && `Floor ${flat.floor}`}
                                            {blockName(flat.block) && ` · ${blockName(flat.block)}`}
                                        </p>
                                    </div>
                                </TiltCard>
                            ))}
