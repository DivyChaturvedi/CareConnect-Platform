...
/* ------------------------------------------------------------------ */
/* Design tokens — shared language across all CareConnect pages        */
/* ------------------------------------------------------------------ */
const COLORS = {
    bg: "#0a0e1a",
...
const emptyForm = { name: "", address: "", city: "", state: "", pincode: "" };
...
function SocietyManagement() {
    ...
    const fetchSocieties = async () => {
        try {
            ...
            console.log(error);
            showToast("error", "Couldn't load societies. Try refreshing.");
        }
    };

    useEffect(() => {
        fetchSocieties().finally(() => setLoading(false));
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log(error);
            showToast("error", "Couldn't create the society. Please try again.");
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <div style={styles.page}>
            ...
            <div style={styles.container}>
                ...
                <div className="cc-fade-in" style={{ ...styles.header, animationDelay: "0ms" }}>
                    ...
                    <div>
                        <h1 className="cc-heading">Society Management</h1>
                        <p style={styles.subText}>Create and manage the communities you look after</p>
                    </div>
                </div>
                ...
                <TiltCard max={3} className="cc-fade-in cc-card" style={{ animationDelay: "80ms" }}>
                    ...
                    <h3 style={styles.cardTitle}>New Society</h3>

                    <form onSubmit={handleSubmit} style={{ position: "relative", zIndex: 1 }}>
                        <div style={{ marginBottom: "16px" }}>
                            <label className="cc-label">Society name</label>
                            <input
                                type="text"
                                name="name"
                                className="cc-input"
                                placeholder="e.g. Meadow Heights"
                                value={formData.name}
                ...
                <div className="cc-fade-in" style={{ animationDelay: "160ms" }}>
                    <h3 style={{ ...styles.cardTitle, margin: "8px 4px 14px" }}>
                        Society List {!loading && <span className="cc-count-badge">{societies.length}</span>}
                    </h3>
                    ...
                    {loading ? (
                        ...
                        <div className="cc-card cc-empty">
                            ...
                            <p style={{ color: COLORS.muted, margin: "10px 0 0", position: "relative", zIndex: 1 }}>
                                No societies yet — create one above to get started.
                            </p>
                        </div>
                    ) : (
                        <div style={styles.grid}>
                            {societies.map((society, i) => (
                                <TiltCard
                                    key={society.id}
                                    max={9}
                                    className={`cc-card cc-society-card ${deletingId === society.id ? "cc-card-removing" : ""}`}
                                    style={{ animationDelay: `${200 + i * 60}ms` }}
                                >
                                    <div className="cc-card-glow" />
                                    <div className="cc-society-icon">🏢</div>

                                    <div style={{ flex: 1, position: "relative", zIndex: 1 }}>
                                        <h3 className="cc-society-name">{society.name}</h3>
                                        <p className="cc-society-meta">
                                            {[society.city, society.state].filter(Boolean).join(", ")}
                                        </p>
                                    </div>

                                    {confirmId === society.id ? (
                                        <div className="cc-confirm-row">
                                            <button className="cc-confirm-yes" onClick={() => handleDelete(society.id)}>
                                                Confirm
                                            </button>
                                            <button className="cc-confirm-no" onClick={() => setConfirmId(null)}>
                                                Cancel
                                            </button>
                                        </div>
                                    ) : (
                                        <button
                                            onClick={() => setConfirmId(society.id)}
                                            disabled={deletingId === society.id}
                                            className="cc-delete-btn"
                                            aria-label={`Delete ${society.name}`}
                                        >
                                            {deletingId === society.id ? <span className="cc-spinner cc-spinner-danger" /> : "Delete"}
                                        </button>
                                    )}
                                </TiltCard>
                            ))}
...
function GlobalStyles() {
    return (
        <style>{`
            ...
            .cc-confirm-yes:hover { box-shadow: 0 8px 20px -8px rgba(251,90,106,0.6); }
            .cc-confirm-no {
                background: rgba(255,255,255,0.06);
                color: ${COLORS.muted};
                border-color: ${COLORS.border};
            }
            .cc-confirm-no:hover { background: rgba(255,255,255,0.1); color: ${COLORS.text}; }
            ...
            .cc-toast {
                ...
                animation: cc-toast-in 0.4s cubic-bezier(0.22,1,0.36,1) both;
            }
            .cc-toast-success span { color: ${COLORS.success}; font-weight: 800; }
            .cc-toast-error span { color: ${COLORS.danger}; font-weight: 800; }
            ...
            @media (prefers-reduced-motion: reduce) {
                .cc-fade-in, .cc-blob, .cc-header-icon, .cc-skeleton, .cc-toast { animation: none !important; }
